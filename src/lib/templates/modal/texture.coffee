define ->

  Handlebars.compile """
  <header class="origin-left">
    <button class="tl-upload">Upload Texture...</button>
  </header>

  <section>
    <ul class="tl-list">

      <li class="tl-entry">
        <div class="tl-entry-img">
          <div style="background-image: url(http://lorempixel.com/256/256)"></div>
        </div>
        <span class="tl-entry-filename">kitten_1.png</span>
        <span class="tl-entry-dimensions">200 x 300 px</span>
        <span class="tl-entry-filesize">234KB</span>
      </li>

      <li class="tl-entry">
        <div class="tl-entry-img">
          <div style="background-image: url(http://lorempixel.com/200/256)"></div>
        </div>
        <span class="tl-entry-filename">kitten_1.png</span>
        <span class="tl-entry-dimensions">200 x 300 px</span>
        <span class="tl-entry-filesize">234KB</span>
      </li>

      <li class="tl-entry">
        <div class="tl-entry-img">
          <div style="background-image: url(http://lorempixel.com/150/200)"></div>
        </div>
        <span class="tl-entry-filename">kitten_1.png</span>
        <span class="tl-entry-dimensions">200 x 300 px</span>
        <span class="tl-entry-filesize">234KB</span>
      </li>

      <li class="tl-entry">
        <div class="tl-entry-img">
          <div style="background-image: url(http://lorempixel.com/512/256)"></div>
        </div>
        <span class="tl-entry-filename">kitten_1.png</span>
        <span class="tl-entry-dimensions">200 x 300 px</span>
        <span class="tl-entry-filesize">234KB</span>
      </li>

      <li class="tl-entry">
        <div class="tl-entry-img">
          <div style="background-image: url(http://lorempixel.com/300/300)"></div>
        </div>
        <span class="tl-entry-filename">kitten_1.png</span>
        <span class="tl-entry-dimensions">200 x 300 px</span>
        <span class="tl-entry-filesize">234KB</span>
      </li>

      <li class="tl-entry">
        <div class="tl-entry-img">
          <div style="background-image: url(http://lorempixel.com/256/256)"></div>
        </div>
        <span class="tl-entry-filename">kitten_1.png</span>
        <span class="tl-entry-dimensions">200 x 300 px</span>
        <span class="tl-entry-filesize">234KB</span>
      </li>

      <li class="tl-entry">
        <div class="tl-entry-img">
          <div style="background-image: url(http://lorempixel.com/200/256)"></div>
        </div>
        <span class="tl-entry-filename">kitten_1.png</span>
        <span class="tl-entry-dimensions">200 x 300 px</span>
        <span class="tl-entry-filesize">234KB</span>
      </li>

      <li class="tl-entry">
        <div class="tl-entry-img">
          <div style="background-image: url(http://lorempixel.com/150/200)"></div>
        </div>
        <span class="tl-entry-filename">kitten_1.png</span>
        <span class="tl-entry-dimensions">200 x 300 px</span>
        <span class="tl-entry-filesize">234KB</span>
      </li>

      <li class="tl-entry">
        <div class="tl-entry-img">
          <div style="background-image: url(http://lorempixel.com/512/256)"></div>
        </div>
        <span class="tl-entry-filename">kitten_1.png</span>
        <span class="tl-entry-dimensions">200 x 300 px</span>
        <span class="tl-entry-filesize">234KB</span>
      </li>

      <li class="tl-entry">
        <div class="tl-entry-img">
          <div style="background-image: url(http://lorempixel.com/300/300)"></div>
        </div>
        <span class="tl-entry-filename">kitten_1.png</span>
        <span class="tl-entry-dimensions">200 x 300 px</span>
        <span class="tl-entry-filesize">234KB</span>
      </li>

    </ul>
  </section>
  """
